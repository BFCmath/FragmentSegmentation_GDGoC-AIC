...
- **Dual Segmentation Modes:**
  - **Fast (RGB):** Uses a YOLO ONNX model optimized for speed, processing standard RGB images.
  - **Precise (RGB-D):** Employs a YOLO ONNX model that leverages depth information (generated by the Depth Anything V2 model) alongside RGB data for potentially more accurate segmentation.
- **API Endpoints:**
...
- `utils/`: Contains utility modules.
  - `depth_handler.py`: Manages the Depth Anything V2 model, including loading the model, preprocessing images for depth estimation, running inference, and postprocessing the depth map.
  - `depth_anything_v2/`: Contains the source code and any associated files for the Depth Anything V2 model.
